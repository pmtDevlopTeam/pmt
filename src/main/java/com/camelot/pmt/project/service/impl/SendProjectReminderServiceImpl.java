...
public class SendProjectReminderServiceImpl implements SendProjectReminderService{
    ...
    /**
     *  实现步骤：
     *  1.根据项目id，角色id查询对应规则的数据，
     *  2.将对应的数据的项目id，角色id，角色下人员id，email等信息插入日志表，
     */
    ...
    public List<List<RemindReport>> queryListByProidAndDate(Long projectId,String remindFrequency){
        ...
        for (int i = 0; i < split.length; i++) {
            if ("01".equals(split[i])){
                ...
                //获取数据信息
                //List<RemindReport> oneDayremindReports = queryRemindReportListByProIdAndDate(projectId, stringBeginDayOfYesterday, stringEndDayOfYesterDay);
            ...
            if ("02".equals(split[i])){
                ...
                //获取数据信息
                //List<RemindReport> weekRemindReports = queryRemindReportListByProIdAndDate(projectId, stringBeginDayOfWeek, stringEndDayOfWeek);
                List<RemindReport> weekRemindReports =null;
                datalists.add(weekRemindReports);
            }
            if ("03".equals(split[i])){
                // 获取上月月起止时间
                // String stringBeginDayOfLastMonth = GetDateUtil.getStringBeginDayOfLastMonth();
                ...
                //获取数据信息
                //List<RemindReport> monthRemindReports = queryRemindReportListByProIdAndDate(projectId, stringBeginDayOfMonth, stringEndDayOfMonth);
     ...
     * @return
     */
    public List<RemindReport> queryRemindReportListByProIdAndDate(Long projectId,String projectRoleId, String startDate, String endDate){
        //List<RemindReport> remindReportList = remindReportMapper.queryRemindReportListByProIdAndDate(projectId,projectRoleId, startDate, endDate);
