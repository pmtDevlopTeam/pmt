...
public class OrgController {
    ...
    public JSONObject addOrg(@ApiIgnore Org org) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(org.getOrgId()) && StringUtils.isEmpty(org.getOrgCode())
                    && StringUtils.isEmpty(org.getOrgname()) && StringUtils.isEmpty(org.getSortNum())
                    && StringUtils.isEmpty(org.getState())) {
                ApiResponse.jsonData(APIStatus.ERROR_400);
            }
    ...
    public JSONObject updateOrgByOrgId(@ApiIgnore Org org) {
        ...
        try {
            ...
            org.setCreatUserId(user.getUserId());
            org.setModifyUserId(user.getUserId());
            if (StringUtils.isEmpty(org.getOrgname()) && StringUtils.isEmpty(org.getOrgId())
                    && StringUtils.isEmpty(org.getOrgCode())) {
                ApiResponse.jsonData(APIStatus.ERROR_400);
            }
    ...
    public JSONObject addOrgToUser(@ApiIgnore OrgToUser orgToUser) {
        try {
            User user = (User) ShiroUtils.getSessionAttribute("user");
            if (StringUtils.isEmpty(user.getUserId())) {
                ApiResponse.jsonData(APIStatus.UNAUTHORIZED_401);
            }
            orgToUser.setCreateUserId(user.getUserId());
            orgToUser.setModifyUserId(user.getUserId());
            if (StringUtils.isEmpty(orgToUser.getOrgId()) && StringUtils.isEmpty(orgToUser.getUserId())
                    && StringUtils.isEmpty(orgToUser.getUserIds())) {
                return ApiResponse.jsonData(APIStatus.ERROR_400);
            }
            return ApiResponse.success(orgService.addOrgToUser(orgToUser));
        } catch (Exception e) {
            logger.error(e.getMessage());
            return ApiResponse.jsonData(APIStatus.ERROR_500, e.getMessage());
        }
    ...
    public JSONObject updateOrgToUser(@ApiIgnore OrgToUser orgToUser) {
        try {
            User user = (User) ShiroUtils.getSessionAttribute("user");
            if (StringUtils.isEmpty(user.getUserId())) {
                ApiResponse.jsonData(APIStatus.UNAUTHORIZED_401);
            ...
            return ApiResponse.success(orgService.updateOrgToUser(orgToUser));
        } catch (Exception e) {
            logger.error(e.getMessage());
            return ApiResponse.jsonData(APIStatus.ERROR_500, e.getMessage());
        }
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "orgLeadername", value = "部门负责人", required = false, paramType = "form", dataType = "String"),
            @ApiImplicitParam(name = "pageNum", value = "页码", defaultValue = "1", required = true, paramType = "query", dataType = "int"),
            @ApiImplicitParam(name = "pageSize", value = "每页数量", defaultValue = "10", required = true, paramType = "query", dataType = "int") })
