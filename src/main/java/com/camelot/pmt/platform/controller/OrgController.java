...
/**
 * @author pmt
 * @Description: 基础平台-组织管理管理接口 注意：所有方法按照增删改查顺序添加
 * @date 2018-04-11
 */
@RestController
...
public class OrgController {
    ...
    public JSONObject creatOrg(@ApiIgnore Org org) {
        ...
        try {
            /*
             * User user = (User) ShiroUtils.getSessionAttribute("user"); if
             * (StringUtils.isEmpty(user.getUserId())) {
             * ApiResponse.jsonData(APIStatus.UNAUTHORIZED_401); }
             * org.setCreatUserId(user.getUserId()); org.setModifyUserId(user.getUserId());
             */
    ...
    public JSONObject addOrgToUser(@ApiIgnore Org org) {
        ...
        try {
            if (StringUtils.isEmpty(org.getOrgId()) && StringUtils.isEmpty(org.getUserId())
                    && StringUtils.isEmpty(org.getUserIds())) {
    ...
    public JSONObject queryOrgToUser(@ApiIgnore Org org) {
        ...
        try {
            if (StringUtils.isEmpty(org.getOrgId()) && StringUtils.isEmpty(org.getOrgCode())
                    && StringUtils.isEmpty(org.getOrgname())) {
                return ApiResponse.jsonData(APIStatus.ERROR_400);
            }
