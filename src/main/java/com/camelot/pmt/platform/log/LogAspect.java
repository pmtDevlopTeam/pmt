            ...
            for (Object map : maps.entrySet()) {
                for (Object newMap : newMaps.entrySet()) {
                    if (((Map.Entry) map).getKey() == ((Map.Entry) newMap).getKey()) {
                        if (((Map.Entry) newMap).getValue() != null
                                && !((Map.Entry) newMap).getValue().equals("null")) {
                            if (((Map.Entry) map).getValue() != ((Map.Entry) newMap).getValue()
                                    && !((Map.Entry) newMap).getValue().equals(((Map.Entry) map).getValue())) {
                                stringBuffer.append(((Map.Entry) map).getKey());
                                stringBuffer.append("由'" + ((Map.Entry) map).getValue() + "'修改为:"
                                        + ((Map.Entry) newMap).getValue() + ",");
                            }
                        }
                    }
                }
